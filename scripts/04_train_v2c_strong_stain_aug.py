...
# ===================================================================
V2_PHASE = "A"
V2_TAG = "v2.0c-phaseA-strongStainAug"
...
def _install_stain_augmentation() -> None:
    ...
    def _patched_get_augmentation_dict(*args, **kwargs):
        ...
        # AGGRESSIVE: probability 0.5, amount 1.0 (full strength)
        aug_probability = 0.5
        aug_amount = 1.0
            ...
            for modality in result[phase]:
                ...
                for key in stain_keys:
                    if key in aug_dict:
                        ...
                        # Override BOTH probability and amount
                        aug_dict[key] = [aug_probability, aug_amount]
                        logger.info(
                            "[v2.0c] AGGRESSIVE %s: %s → %s",
...
def _log_preamble(cfg: dict, cfg_hash: str) -> None:
    ...
    logger.info("Stain augment     : AGGRESSIVE (prob=0.5, amount=1.0)")
    cfg["training"]["experiment_name"] = "brightfield_cells_nuclei_strongStainAug"
...
def main() -> None:
    ...
    # 2. Stain augmentation — AGGRESSIVE for v2.0c ablation
    _install_stain_augmentation()
    logger.info("[v2.0c] Stain augmentation AGGRESSIVE (prob=0.5, amount=1.0)")
